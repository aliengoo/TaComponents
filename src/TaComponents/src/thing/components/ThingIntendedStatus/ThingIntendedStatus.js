...
export default class ThingIntendedStatus extends React.Component {
  ...
  render() {
    ...
    const options = {
      errorsMap: {
        valueMissing: "You must select the intended status"
      },
      fieldSetter,
      label: (<span>Intended status <RequiredIndicator/></span>),
      labelKey: "text",
      name: "intendedStatusId",
      options: statuses,
      tooltip: (
        <Tooltip container="ThingIntendedStatus">
          <p>What is the intended status of this thing?</p>
        </Tooltip>
      ),
      value,
      validator: (value) => {
